...
export default {
  ...
  methods: {
    ...
    deletePage(item) {
      if (
        this.activePageUUID === item.uuid &&
        this.projectData.pages.length > 1
      ) {
        // console.log("vue:",this.projectData.pages.indexOf(item));
        this.$store.dispatch(
          'template/setActivePageUUID',
          this.projectData.pages[this.projectData.pages.indexOf(item) - 1].uuid
        );
      }
    ...
    inputChangeImg() {
      testTemplateData.pages[0].elements[0].propsValue.imageSrcList[0] =
        this.img;
    }
  }
};
