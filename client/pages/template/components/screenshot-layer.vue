<template>
  <div ref="con" class="screenshot-layer">
    <div
      ...
      :key="page.uuid"
      :style="{
        width: pagesData.width + 'px',
        height: pagesData.height + 'px'
      }"
      class="slayer-page"
    >
      <div
...
export default {
  ...
  methods: {
    publishFun() {
      ...
      //todo -修改发布视频的数据:模板测试
      this.pagesData = templateProjectConfig.processingProjectData(
        this.projectData,
        true
      );
    ...
    updateVideoData(result) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        // console.log("-----------------------------------")
        // console.log("this.videoData:",this.videoData)
        const element = templateProjectConfig.getDataByKeyFromVideoData(
          this.videoData,
          id
        ); //通过id获取元素
        if (element) {
    ...
    async beginMakingVideo(videoData) {
      // console.log(videoData);
      const res = await this.$API.templateBeginMakeVideo({
        videoData,
    ...
    getProgressing({ taskId, uuid }) {
      ...
      const id = setInterval(async () => {
        const res = await this.$API.templateGetVideoPercent({
          taskId,
          uuid,
          id: this.$route.query.id
        });
    ...
    async screenshots(elements, i) {
      ...
      forEach(canvass, (canvas, j) => {
        ...
        const file = this.$mUtils.canvasToFile({
          name,
          canvas,
          quality: 0.9,
          type: 'png'
    ...
    groupingPagesData(pagesData) {
      ...
      const donotMergeLayer = (ele) => {
        ...
        return (
          !isEmpty(ele.animations) ||
          ele.elName === 'qk-video' ||
          ele.elName === 'qk-image-carousel' ||
          ele.elName === 'qk-text' ||
          isGIF
        );
      };
      ...
      for (let i = 0; i < domIdsArr.length; i++) {
        ...
        for (let j = 0; j < eleArr.length; j++) {
          ...
          if (isArray(eles)) {
            ...
            $(con).find(`#slayer-pw-${i}`).append(elesCon);
            ...
            // 隐藏
            $(con).find(`#slayer-e-${eles}`).css('display', 'none');
          }
        }
      }
