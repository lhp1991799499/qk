  ...
  methods: {
    ...
    updateVideoData(result) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        const element = editorProjectConfig.getDataByKeyFromVideoData(
          this.videoData,
          id
        ); //通过id获取元素
        if (element) {
    ...
    async beginMakingVideo(videoData) {
      const res = await this.$API.beginMakeVideo({
        videoData,
    ...
    getProgressing({ taskId, uuid }) {
      ...
      const id = setInterval(async () => {
        const res = await this.$API.getVideoPercent({
          taskId,
          uuid,
          id: this.user.userInfo._id
        });
    ...
    async uploadAllImages(pfiles) {
      ...
      params.append('id', String(this.user.userInfo._id));
