...
export default {
  ...
  mounted() {
    // 监听元素移动和不移动的事件
    this.$bus.$on('move', (isDownward, isRightward) => {
  ...
  methods: {
    // 改变缩放比例后的样式
    styleScalce(style, arr) {
      arr.forEach((item) => {
        style[item] *= this.transformScalce;
      });
    },
    ...
    showLine(isDownward, isRightward) {
      ...
      const components = this.activePage;
      const curComponentStyle = getComponentRotatedStyle(
        this.activeElement.commonStyle
      );
      let arr = ['height', 'width'];
      //   this.styleScalce(curComponentStyle, arr);
      const curComponentHalfwidth = curComponentStyle.width / 2;
      const curComponentHalfHeight = curComponentStyle.height / 2;

      this.hideLine();
      components.elements.forEach((component) => {
        if (component == this.activeElement) return;
        const componentStyle = getComponentRotatedStyle(component.commonStyle);
        let arr = ['left', 'right', 'height', 'width'];
        // this.styleScalce(componentStyle, arr);
        ...
        const needToShow = [];
        const { rotate } = this.activeElement.commonStyle;
        Object.keys(conditions).forEach((key) => {
          ...
          conditions[key].forEach((condition) => {
            ...
            condition.lineShift *= this.transformScalce;
    ...
    translatecurComponentShift(key, condition, curComponentStyle) {
      const { width, height } = this.activeElement.commonStyle;
      if (key == 'top') {
        return Math.round(
